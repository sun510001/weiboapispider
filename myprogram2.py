...
def access_token_fac():
    ...
    if len_key == 4:
        ...
        with open('key.txt', 'w') as w_key:
            w_key.write(APP_KEY + '\n')
        with open('key.txt', 'a') as a_key:
...
def check_flag():
    # check the stop_flag:
    check_stop_flag_file = Path('stop_flag.txt')
    if check_stop_flag_file.exists():
        with open('stop_flag.txt', 'r') as f_stop:
        ...
        if not stop_flag == 'False':
            ...
            with open('stop_flag.txt', 'w') as w_stop:
                w_stop.write('False')
    else:
        with open('stop_flag.txt', 'w') as w_stop:
            w_stop.write('False')
